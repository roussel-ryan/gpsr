...
def add_projection(ax, key, beams, bins, label="", plot_samples=False):
    histograms = []
    for ele in beams:
        histograms += [
            np.histogram(
    ...
    ax.plot(bins[:-1].cpu(), means, label=key + label)
    ax.fill_between(bins[:-1].cpu(), l, u, alpha=0.5)
    
    if plot_samples:
        for ele in histograms:
            ax.plot(bins[:-1].cpu(), ele)
