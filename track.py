...
def make_track_a_quadrupole(lib):
    ...
    def low_energy_z_correction(pz, p0c, mass, ds):
        ...
        dz = (
            ...
            * (
                1
                - 3 * (pz * beta0 ** 2) / 2
                + pz ** 2 * beta0 ** 2 * (2 * beta0 ** 2 - (mass / e_tot) ** 2 / 2)
            )
            * (mass / e_tot) ** 2
        ) * (evaluation < 3e-7 * e_tot) + (ds * (beta - beta0) / beta0) * (
            evaluation >= 3e-7 * e_tot
        )

        return dz

    def offset_particle_entrance(x_offset, y_offset, x_lab, y_lab, px_lab, py_lab):
    ...
    def offset_particle_exit(x_offset, y_offset, x_ele, y_ele, px_ele, py_ele):
    ...
    def track_a_quadrupole(p_in, quad):
        ...
        for i in range(n_step):
            ...
            tx11, tx12, tx21, tx22, dz_x1, dz_x2, dz_x3 = quad_mat2_calc(
                -k1, step_len, rel_p
            )
            ty11, ty12, ty21, ty22, dz_y1, dz_y2, dz_y3 = quad_mat2_calc(
                k1, step_len, rel_p
            )
