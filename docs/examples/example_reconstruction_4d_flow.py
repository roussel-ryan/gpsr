...
exists. Failure to fit the data likely points to an incorrect forward model.
(It is also possible, but unlikely, that the NN cannot represent the true 
distribution.) An entropy-regularized solution may require more iterations 
with a more expensive generative model, so this initial training should not
add much to the overall training time. Second, we want to estimate the 
covariance matrix of the distribution. Flow-based generative models perform
best when the scale or covariance of the distribution is known. For example,
in density estimation, samples are typically scaled to identity covariance 
...
want to compare the NN solution to the entropy-regularized solution to see
if there is any difference in features.

After estimating the covariance matrix, we train a normalizing flow to 
minimize the KL divergence between the reconstructed distribution 
:math:`p_{\theta}(x)` and the prior distribution :math:`q(x)`, where 
:math:`\theta` represents the generative model parameters. The KL 
divergence is the negative of the relative entropy, which we write as 
