...
class CustomObservableDataset(Dataset):
    """
    Define a PyTorch dataset to be passed to a TorchDataloader object for training.
    ...
    def __init__(self, parameters: Tensor, observations: Tuple[Tensor, ...]):
        ...
        if len(self.observations) > 1:
            ...
            # we have to flatten any batch dimensions B for batching purposes
            self._flattened_parameters = torch.flatten(
                parameters, start_dim=0, end_dim=-2
            )
            self._flattened_observations = tuple(
                [
                    torch.flatten(ele, end_dim=-3)
                    for ele in self.observations
                ]
            )
        else:
...
class LinacQuadDipoleDataset(CustomObservableDataset):
    def __init__(
        self,
        parameters: Tensor,
        observations: Tuple[Tensor, Tensor],
        ...
        """
        Light wrapper dataset class for 6D phase space reconstructions with linac, quadrupole 
        and dipole. Checks for correct sizes of parameters and observations.

        Parameters
        ----------
        parameters : Tensor
            Tensor of beamline parameters that correspond to data observations.
            Elements along the last dimension should be ordered by 
            (linac phases, quadrupole focusing strengths) and should have a
            shape of (N x M x 2) where N is the number of linac phases and M is the
            number of quadrupole strengths.
        observations : Tuple[Tensor, Tensor]
            Tuple of tensors containing observed images, where the tensor shapes
            should be (N x M x n_bins x n_bins). The first entry should correspond to 
            screen 1 and the second to screen 2.
        bins: Tuple[Tensor, Tensor]
            Tuple of 1-D bin locations for each image set. Assumes square images with
            the same bin locations.
        """

        assert parameters.shape[-1] == torch.Size([2])
        assert len(observations) == 2
