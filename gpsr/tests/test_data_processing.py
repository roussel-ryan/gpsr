...
class TestProcessImages:
    ...
    def test_process_images_no_pooling_no_filter(self, image_shape):
        ...
        processed_images, meshgrid = process_images(images, screen_resolution, self.mock_image_fitter)
        ...
        assert np.allclose(meshgrid[0][0], np.linspace(-8 * self.rms_size[0], 8 * self.rms_size[0] - 1., 8 * 2 * self.rms_size[0])*1.0e-6)
    ...
    def test_process_images_with_pooling(self, image_shape):
        ...
        screen_resolution = 1.0
        pool_size = 2
        processed_images, meshgrid = process_images(images, screen_resolution, self.mock_image_fitter, pool_size=pool_size)

        assert processed_images.shape == image_shape[:-2] + tuple(8 * 2 * self.rms_size // pool_size)
        ...
        assert np.allclose(meshgrid[0][0], pool_size * np.linspace(-size / 2, size / 2 - 1., size)*1.0e-6)
    ...
    def test_process_images_with_median_filter(self, image_shape):
        ...
        process_images(images, screen_resolution, self.mock_image_fitter, median_filter_size=median_filter_size)
    ...
    def test_process_images_with_threshold(self, image_shape):
        ...
        screen_resolution = 1.0
        threshold = 0.5
        process_images(images, screen_resolution, self.mock_image_fitter, threshold=threshold)
