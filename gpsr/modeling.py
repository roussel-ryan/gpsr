...
class GeneralizedGPSRLattice(GPSRLattice):
    ...
    def __init__(self, 
        ...
        for element in variable_elements:
            if not hasattr(element[0], element[1]):
                raise AttributeError(f"Variable element {element[0].name} does not have parameter '{element[1]}'.")
        
        for element in observable_elements:
            if not hasattr(element, 'reading'):
                raise AttributeError(f"Observable element {element.name} does not have a 'reading' property.")
    ...
    def track_and_observe(self, beam) -> Tuple[Tensor, ...]:
        ...
        final_beam = merged_lattice(beam)
