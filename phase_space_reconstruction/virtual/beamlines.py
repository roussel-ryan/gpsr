...
def doublet_quad_tdc_bend(p0c, dipole_on=False):
    ...
    # Drift from Quad to TDC (0.5975)
    l_d3 = 0.790702 - l_q/2 - l_tdc/2

    # Drift from TDC to Bend (0.3392)
    l_d4 = 0.631698 - l_tdc/2 - l_bend/2

    # Drift from Bend to YAG 2 (corrected for dipole on/off)
    l_d5 = 0.889 - l_bend/2/np.cos(theta)
    ...
    d1 = TorchDrift(L = torch.tensor(l_1))
    ...
    d2 = TorchDrift(L = torch.tensor(l_2))
    ...
    lattice = TorchLattice([q1, d1, q2, d2, q3, d3, tdc, d4, bend, d5])
    
    return lattice

def triplet_quad_tdc_bend(p0c):
    ...
    d3 = TorchDrift(L = torch.tensor(l_3))
