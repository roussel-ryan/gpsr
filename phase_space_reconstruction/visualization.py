...
def plot_6D_reconstruction_data(
        train_dset: SixDReconstructionDataset, publication_size:
        bool = False
):
    """
    ...
    n_g, n_v, n_k = train_dset.parameters.shape[:-1]
    params = train_dset.parameters
    images = train_dset.observations
    bins = train_dset.bins
...
def plot_projections(
    ...
    for coord in coords:
        all_coords.append(getattr(particles, coord).cpu().detach())
    ...
    for i in range(n_coords):
        ...
        if x_coord == 'pz':
            x_array = getattr(particles, x_coord).cpu().detach() * 100
            ...
            x_array = getattr(particles, x_coord).cpu().detach() * scale
        ...
        for j in range(i + 1, n_coords):
            ...
            if y_coord == 'pz':
                y_array = getattr(particles, y_coord).cpu().detach() * 100
                min_y = coord_min[j] * 100
                max_y = coord_max[j] * 100

            else:
                y_array = getattr(particles, y_coord).cpu().detach() * scale
